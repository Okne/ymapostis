...
#import codecs
#lc.setlocale(lc.LC_ALL, ('en_US', '1251'))
...
class Pair(Arc):
    ...
    def write_to_scs(self):
        ...
        if defines.NEW_SC_MODEL or defines.NEW_STYLE_BINARY_PAIRS:
            res = "$%s%s = (%s %s%s %s);" % (_pair_pre,
                                            self.glob_id,
                                            self.globid2el[self.loc2glob_id[self._beg]].get_glob_id_str(),
                                            "", _pair_const,
                                            self.globid2el[self.loc2glob_id[self._end]].get_glob_id_str())
